...
function magePassive(
  ...
  targetPiece.trappedTurns = MAGE_BIND_TURNS;
  targetPiece.netDamage = 0; // 束縛只定身，不像撒網會持續扣血
...
function fearedTarget(
  ...
  // 一定要用「正在行動的座位」而不是 seats.indexOf(playerId)：
  // 代打 NPC 時 playerId 是操作者，恐懼會算到操作者自己的角色頭上。
  const fear = state.seats[activeSeat]?.fearTurns ?? 0;
...
function movementInterrupted(
   ...
   * 同樣要看「正在行動的座位」。
   *
   * 舊寫法用 seats.indexOf(playerId)，代打 NPC 時查到的是操作者自己的座位 ——
   * 被踩到的陷阱把 NPC 放逐了，這裡卻回報「沒被打斷」，於是回合不收尾、
   * 繼續拿一個已經不在棋盤上的棋子往下跑。畫面上就是那個座位卡住、
   * 只能等 45 秒讀秒結束。
   */
  const banished = state.seats[activeSeat]?.banishedTurns;
